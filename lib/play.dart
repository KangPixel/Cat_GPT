...
class _PlayScreenState extends State<PlayScreen> {
              ...
              Expanded(
                child: GridView.count(
                  ...
                  children: [
                    ...
                    _buildGameCard('WATERMELON GAME', 'assets/images/watermelon.png', () async {
                      try {
                        debugPrint('Suika Game 실행 시작...');
                        final game = MainGame();

                        if (game == null) {
                          throw Exception('Suika Game을 불러올 수 없습니다.');
                        }

                        debugPrint('Suika Game 로드 성공! 게임 리소스 로드 중...');

                        await game.onLoad(); //onload를 직접 호출하여 비동기 로드 완료 후 진행
                        ...
                        Future.delayed(const Duration(milliseconds: 500), () {
                         ...
                         Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => _buildGameScreen(context, game, 'suika_game'),
                            ),
                          );
                        });
                      } catch(e) {
                        debugPrint('게임 로드 중 오류 발생: $e');
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text('게임 로드 중 오류 발생: $e')),
                        );
                      }
                    }),
                 ],
  ...
  Widget _buildGameScreen(BuildContext context, FlameGame game, String title) {
    return FutureBuilder<void>(
      future: Future.value(game.onLoad()), // 🔥 `onLoad()` 완료될 때까지 기다림
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return Scaffold(
            appBar: AppBar(title: Text('$title (로딩 중...)')),
            body: const Center(child: CircularProgressIndicator()), // 🔄 게임 로딩 중 스피너 표시
          );
        } else if (snapshot.hasError) {
          return Scaffold(
            appBar: AppBar(title: Text('$title (오류 발생)')),
            body: Center(child: Text('게임 로드 중 오류 발생: ${snapshot.error}')),
          );
        }
